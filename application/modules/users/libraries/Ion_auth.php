...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$this->_cache_role_in_group =& $this->ion_auth_model->_cache_role_in_group;
	...
	public function in_role($check_group, $id=false, $check_all = false)
	{
		$this->ion_auth_model->trigger_events('in_role');
		...
		if (isset($this->_cache_role_in_group[$id]))
		{
			$groups_array = $this->_cache_role_in_group[$id];
		}
		else
		{
			$roles_groups = $this->ion_auth_model->get_roles_groups($id)->result();
			$groups_array = array();
			foreach ($roles_groups as $group)
			{
				$groups_array[$group->id] = $group->name;
			}
			$this->_cache_role_in_group[$id] = $groups_array;
		}
		foreach ($check_group as $key => $value)
		{
			$groups = (is_string($value)) ? $groups_array : array_keys($groups_array);
