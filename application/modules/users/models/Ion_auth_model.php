...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @var array
	 **/
	public $_cache_role_in_group = array();
	...
	public function __construct()
	{
		...
		$this->join2			   = $this->config->item('join2', 'ion_auth');
	...
	{
		$this->trigger_events('get_roles_groups');

		// if no id was passed use the current users id
		$id || $id = $this->ion_auth->user()->row()->roleID;

		return $this->db->select($this->tables['roles_groups'].'.'.$this->join2['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['roles_groups'].'.'.$this->join2['roles'], $id)
		                ->join($this->tables['groups'], $this->tables['roles_groups'].'.'.$this->join2['groups'].'='.$this->tables['groups'].'.id')
		                ->get($this->tables['roles_groups']);
	}
	...
	public function rolesoptions()
	{
		...
		if($result->num_rows() > 0){
				...
				foreach($result->result_array() as $row){
						$return[$row['roleID']] = $row['roleAlias'];
				}
		}
		return $return;
	}
